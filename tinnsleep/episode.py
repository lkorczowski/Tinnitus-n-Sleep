class episode():
    """Define a bruxism episode object and caracterize it

    Parameters
    ----------
    beg : time of beggining of the episode
    end : time of end of the episode
    

    Attributes
    ----------
    nb_bursts: int, nb of bursts within the episode
    burst_list: list, list of burst(as simple episode objects) contained within
    an episode
    is_tonic: boolean, caracterizes if a bruxism episode is tonic i.e superior 
    in duration to 2 seconds.
    is_phasic: boolean, caracterizes if a bruxism episode is phasic
    is_mixed: boolean, caracteerises if a bruxism episode is mixed
    ...
    def add_a_burst(self, burst):
        ...
        if self.burst_list[len(self.burst_list)-1].merge_if_overlap(burst):
        ...
        # Case if it is overlapping
        elif self.burst_list[0].merge_if_overlap(burst):  
    ...
    def assess_type(self):
        """ caracterizes fully the episode"""
        self.set_tonic()
        self.set_phasic()
        self.set_mixed()
